...
def calculate_SAD(img1, img2, pos1, pos2, window):
    res = 0
    for addi in range(-window // 2, window // 2 + 1):
        for addj in range(-window // 2, window // 2 + 1):
...
def estimate_disparity(img1, img2, window, disparity_range):
    ...
    for i in range(len(img1)):
        for j in range(len(img1[i])):
            min_val = (window + 1) * (window + 1) * 256
            for i2 in range(max(0, i + disparity_range[0]), i + disparity_range[1] + 1):
                for j2 in range(max(0, j + disparity_range[2]), j + disparity_range[3] + 1):
